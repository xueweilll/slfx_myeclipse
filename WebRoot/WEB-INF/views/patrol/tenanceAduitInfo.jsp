			...
			<table>
				...
				<tr>
				   <td>	<a id="add" class="easyui-linkbutton" data-options="iconCls:'icon-add'"
					onclick="addsuggestion()">添加审批意见</a></td>
	...
	<script type="text/javascript">
		$(function() {
			$("#tenancedetial_list").datagrid({
				url : 'tenanceAduitList/tenanceAduitInfoList.html?id='+$('#id').val(),
				rownumbers : true,
				width : 'auto',
				height : '200px',
				closed : false,
		...
		function agree(obj){
			...
			$.messager.confirm('确认对话框', '是否审批?', function(r){
				if (r){
					$.ajax({
						type:"POST",
					    url:"tenanceAduitList/agree.html",
					    dataType:"text",
					    data:{
					    	"id":$("#id").val(),
					    	"type":obj,
		...
		function reflush() {
			document.getElementById('tenanceAduitList.htmlifm').contentWindow.$(

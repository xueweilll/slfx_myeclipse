...
.datagrid-row-selected {
	...
	<form id="ff" method="post"	style="margin: 10px; text-align: center;">		
		<div  style="" align="left">
			...
			<div style="float: left;padding-bottom:10px;">
			<label>分解内容:</label>
			<input class="easyui-textbox" id="contents" data-options="multiline:true,width:395,height:60" required="true"  value="${resolveInfo.getContent() }">
			</div>
		</div> 
		<br>
	...
	<script type="text/javascript">
		$(function(){
			$("#stationPatrol").datagrid({
				...
				collapsible : false,
				fit : true,
				url : 'egpatrolengineer/findDepartment.html',
				columns:[[
				...
				onLoadSuccess:function(data){
					$.ajax({
						...
						/* url:'egpatrolengineer/findDetailsByRid.html', */
						type : "POST",
						dataType : "text",
						data : {
							"rid" :$('#id').val()
		...
		function add(obj){
			...
			if(rows.length==0){
				$.messager.alert("操作提示", "请至少选择一个部门再进行操作！", "error");
				return false;
			}else{
			/* for(var i=0;i<rows.length;i++){
				arr.push(rows[i].id);
			...
			$.ajax({
				url : "regularegpatrolengineer/"+(obj=="0" ? "send":"commit")+".html",
				type : "POST",
				dataType : "text",
				data : {
		...
		function reflush(){
			document.getElementById('egpatrolengineer.htmlifm').contentWindow

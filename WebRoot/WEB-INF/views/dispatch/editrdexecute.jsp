...
<body>
	...
	<form id="addrdexecute" method="post"
		...
		<div>
			...
			<input id="executer" style="width:415px;" data-options="required:true"/>
			<input id="userids" type="hidden"  value="${userids}"/>
		</div>
		<div>
			<label for="startouterlevel">开机时外河水位:</label>
			<input class="easyui-textbox" id="startouterlevel" data-options="validType:'intOrFloat'"
				style="width:415px" value="${rde.getStartouterlevel()}" />
		</div>
		<div>
			<label for="startinlandlevel">开机时内河水位:</label>
			<input class="easyui-textbox" id="startinlandlevel" data-options="validType:'intOrFloat'"
				style="width:415px" value="${rde.getStartinlandlevel()}" />
		</div>
		<div>
			<label for="stopouterlevel">停机时外河水位:</label>
			<input class="easyui-textbox" id="stopouterlevel" data-options="validType:'intOrFloat'"
				style="width:415px" value="${rde.getStopouterlevel()}" />
		</div>
		<div>
			<label for="stopinlandlevel">停机时内河水位:</label>
			<input class="easyui-textbox" id="stopinlandlevel" data-options="validType:'intOrFloat'"
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			function onEndEdit1(index, row) {
				...
				if(row.unit  != $(ed1.target).combobox('getValue')){
			...
			$('#gates').datagrid({
				...
				columns : [ [ {
					...
					width : 180
				},{field:'operatetype',title:'',hidden:true},
				{
					field : 'operatetime',
					title : '闸门执行时间',
					editor : {
			...
			function onEndEdit2(index, row) {
				...
				var value = $(ed1.target).combobox('getValue');
				if(value != row.operate){
				...
				if(row.sid != $(ed2.target).combobox('getValue')){

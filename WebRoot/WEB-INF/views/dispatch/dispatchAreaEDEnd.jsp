...
<script type="text/javascript">
	$(function() {
		$('#tt').tabs({
			...
			onSelect : function(title, index) {
				...
				$("#receipt_list").datagrid("load", {
					isAduit:"1",
					receipter:$("#receipter").textbox("getText"),
		...
		$("#receipt_list").datagrid({
			...
			url : 'dispatchAreaEDEnd/dispatchAreaEDEndList.html',
			remoteSort : false,
			idField : 'id',
			singleSelect : true,
			pagination : true,
			...
			onDblClickRow:function(index,row){				
				showDialogWH("查看调度令信息","dispatchAreaEDEnd/dispatchAreaEDEndInfo.html?id=" + row.id +"&look=1", 420, 480);
			}
		});
		...
		$("#btnAudit").bind("click",function() {
			...
			showDialogWH("结束片区调度指令信息","dispatchAreaEDEnd/dispatchAreaEDEndInfo.html?id=" + row.id+"&look=0", 420,480);
			$("#receipt_list").datagrid("unselectAll");
		});
...
<body class="easyui-layout" id="cc">
	...
	<div id="tb"
		...
		<div class="cz_div_title" id="btnList" style="">
			<a id="btnAudit" class="easyui-linkbutton" style=""data-options="plain:false,iconCls:'icon-edit'">结束</a>
		</div>
		<div class="cz_div">

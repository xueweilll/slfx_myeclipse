...
<script type="text/javascript">
	$(function() {
		...
		$("#receipt_list").datagrid({
			...
			fit : true,
			url : 'bigsurroundreceiptfinish/pageBind.html?type='+ $("#type").val(),
			remoteSort : false,
			idField : 'id',
			singleSelect : true,
			pagination : true,
			...
			onDblClickRow : function(index, row) {
				showDialogWH("查看调度令信息",
						"bigsurroundreceiptfinish/bigSurroundReceiptFinishListInfo.html?id="
								+ row.id + "&look=1", 420, 480);
			}
		});
		...
		$("#btnAudit").bind("click",function() {
			...
			showDialogWH("签发片区调度指令信息", "bigsurroundreceiptfinish/bigSurroundReceiptFinishListInfo.html?id="	+ row.id + "&look=0", 420, 480);
			$("#receipt_list").datagrid("unselectAll");
		});
...
<body class="easyui-layout" id="cc">
	...
	<div id="tb"
		...
		<div class="cz_div_title" id="btnList" style="">
			<a id="btnAudit" class="easyui-linkbutton" style=""
				data-options="plain:false,iconCls:'icon-edit'">完成</a>
		</div>
		<div class="cz_div">
			<div style="display:none;">
				调度令编号:&nbsp;<input id="code" class="easyui-textbox"
					style="width: 120px"> &nbsp;&nbsp;
			</div>
			调度令接收人:&nbsp;<input id="receipter" class="easyui-textbox"
				style="width: 120px" />&nbsp;&nbsp; 调度令接收时间区间： &nbsp;<input
				id="begintime" style="width: 160px" /> ~ <input id="endtime"
				class="easyui-datetimebox"
				data-options="width:160,validType:['compareDate[begintime]']" />&nbsp;&nbsp;
			<a href="javascript:void(0)" class="easyui-linkbutton"
				onclick="selectbig()" iconCls="icon-search" id="search">查询</a>
		</div>

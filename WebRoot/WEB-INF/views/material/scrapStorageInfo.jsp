...
<body>
	...
	<form id="ff" method="post"
		...
		<div style="float: left;">
			<div style="float: left;"><label style="width: 70px;text-align: center;">报废单编号:</label><input class="easyui-textbox" id="code" data-options="width:230,disabled:true"  value="${stock.getCode()}" > </div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">报废审批人:</label><input id="handler"  type="text"  ></div>
		</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#handler').combogrid({    
			    ...
			    width:230,
			    textField:'ename',    
			    url:'scrapStorage/userBind.html',
			    required:true,
			    editable:false,
			    columns:[[    
			...
			$('#inStorageInfo').datagrid({
				...
				collapsible : false,
				singleSelect:true,
				url:'scrapStorage/selectStockItems.html',
				rownumbers:true,
				queryParams: {
					id: $("#sid").val()
				},
			    ...
			    columns:[[
				    ...
				    {field:'name',title:'物资名称',width:90,editor:{
							type:'combobox',
							options:{
								url:'scrapStorage/selectMaterials.html',
								required:true,
								editable:false,
								disabled:true,
								valueField:'name',
				    ...
				    {field:'count',title:'报废数量',width:90,editor:{
							type:'textbox',
							options:{
								required:true,
		...
		function add(){
			...
			if(editindex!=undefined){
				...
				$.messager.alert("操作提示", "请保存报废明细！", "error");
				return false;
			}
			...
			if(rows.length==0){
				$.messager.progress('close');
				$.messager.alert("操作提示", "请添加报废明细！", "error");
				return false;
			}else{
				for (var i = 0; i < rows.length; i++) {
		...
		function edit(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存报废数据！", "error");
				return false;
			}
		...
		function apecct(){
			...
			if(isval){
				...
				$.messager.alert("操作提示", "请选择物资或正确填写数量！", "error");
				return false;
			}
			editindex=undefined;
		}
		function addOrder(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存报废明细！", "error");
				return;
			}
			var rows=$("#inStorageInfo").datagrid("getRows");
		...
		function reflush() {
			document.getElementById('scrapStorage.htmlifm').contentWindow.$('#scrapStorage_list')
					.datagrid('reload');
			document.getElementById('scrapStorage.htmlifm').contentWindow.$('#scrapStorage_list')
		...
		function commit(){
			$.messager.confirm('确认对话框', '您确定要提交吗？', function(r){
				if (r){
					$.messager.progress();
					$.post("scrapStorage/commit.html",{"id":$("#sid").val()},function(data){
						data = eval('(' + data + ')');
						$.messager.progress('close');
						if(data.result===true){
							reflush();

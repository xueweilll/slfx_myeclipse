...
<script type="text/javascript">
	...
	$(function() {
		$('#tt').tabs({
			...
			onSelect : function(title, index) {
				...
				$("#checkStorage_list").datagrid("unselectAll");
				$("#checkStorage_list").datagrid("load", {
					typeDate : index
				});
			}
		});
		$("#checkStorage_list").datagrid({
			width : 'auto',
			height : 'auto',
			pageSize : 20,
			nowrap : false,
			...
			columns:[[    
			          ...
			          {field:'proposer',title:'盘点人',width:140},    
			          {field:'proposerTime',title:'盘点时间',width:140}, 
		...
		var p = $("#checkStorage_list").datagrid('getPager');
		//console.info(p);
		$(p).pagination({
			pageList : [ 5, 10, 15, 20 ],
			beforePageText : '第',
	...
	function add() {
		showDialogWH("添加物资盘库", "checkStorage/checkStorageInfo.html?id=0",660,530);
	}

	function edit() {
		var row = $("#checkStorage_list").datagrid("getSelected");
		if (row == null) {
		...
		showDialogWH("编辑物资盘库", "checkStorage/checkStorageInfo.html?id=" + row.id,670,530);
	}
			...
			if (data) {
				...
				$.ajax({
					...
					success : function() {
						...
						$("#checkStorage_list").datagrid("unselectAll");
						$("#checkStorage_list").datagrid("reload");
					}
				});
			}
		});
	...
	function commit(){
		...
		showDialogWH("提交盘库单", "checkStorage/checkStorageInfo.html?id=" + row.id+"&type=1",650,550);
	}
	function searchInfo(row){
		showDialogWH("查看盘库明细", "checkStorage/checkStorageInfo.html?id=" + row.id+"&type=0",650,550);
	}
...
<body class="easyui-layout" id="cc">
	<table id="checkStorage_list" cellspacing="0" cellpadding="0"></table>
		...
		<div class="cz_div">
			...
			盘库编号:<input id="code" class="easyui-textbox" name="code"   

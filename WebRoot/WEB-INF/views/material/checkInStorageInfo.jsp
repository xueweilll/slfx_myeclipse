...
<body>
	...
	<input type="hidden" id="pid" value="${stock.getParentid()}">
	<input type="hidden" id="storageInfo" value="${storageInfo}">
	<input type="hidden" id="peid" value="${stock.getProposer()}">
	<input type="hidden" id="hid" value="${stock.getHandler()}">
	<form id="ff" method="post"
		style="margin: 10px; text-align: center;">
		<div style="float: left;">
			<div style="float: left;"><label style="width: 70px;text-align: center;">归还单编号:</label><input class="easyui-textbox" id="code" data-options="width:230,disabled:true"    value="${stock.getCode()}" > </div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">归还审批人:</label><input id="handler"  type="text" value="" ></div>
		</div>
		<div style="float: left;">
			<div style="float: left;"><label style="width:70px;text-align: center;">外借单编号:</label><input class="easyui-textbox" id="pcode" data-options="width:230" required="true"  value="${stock.getParentid()}" > </div>
		</div>
		<div style="float: left;">
			<div style="float:left;"><label style="width: 70px;text-align: center;">归还人:</label><input id="proposer"  type="text" value="" ></div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">归还时间:</label><input class="easyui-datetimebox" id="proposerTime" data-options="buttons:buttons,width:230,editable:false" required="true"  value="${datetime}" > </div>
		</div>
		...
		<div><table id="pinStorageInfo" cellspacing="0" cellpadding="0"></table></div>
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#handler').combogrid({    
			    ...
			    width:230,
			    textField:'ename',    
			    url:'checkInStorage/userBind.html',
			    required:true,
			    editable:false,
			    columns:[[    
			...
			$('#pcode').combobox({    
			    url:'checkInStorage/selectStock.html',    
			    valueField:'id',    
			    textField:'text',
			    width:230,
			    onSelect:function(record){
			    	$('#pinStorageInfo').datagrid("load",{id:record.id});
			    },
			    onBeforeLoad:function(param){
			    	var id=$("#pid").val();
			    	if(id!=""){
			...
			if($("#sid").val()=="0"){
				$("#proposerTime").datetimebox({value:datetime});
			}
			$('#proposer').combogrid({    
			    //value:'006',    
			    idField:'id',
			    ...
			    onLoadSuccess:function(node){
			    	if(node.rows.length!=0){
			    		for (var i = 0; i < node.rows.length; i++) {
				    		if(node.rows[i].id==$("#peid").val()){
				    			$("#proposer").combogrid("setValue", $("#peid").val());
				    		}
						}
			    	}
			    }    
			...
			if($("#storageInfo").val()=="0"){
				...
				$("#proposer").combogrid({disabled:true,value:$("#peid").val(),onBeforeLoad:function(param){return false;}});
				$("#pcode").combobox({disabled:true,onBeforeLoad:function(){return false;}});
				$("#proposerTime").datetimebox({disabled:true});
			...
			if($("#storageInfo").val()=="1"){
				...
				$("#memo").attr("disabled",true);
				$("#casuse").attr("disabled",true);
				$("#pcode").combobox({disabled:true});
				$("#btnAdd").hide();
				$("#commit").show();
				$("#btnDetails").hide();
			}
			$('#pinStorageInfo').datagrid({
				title : '物资外借明细',
				width : '610',
				height : '200',
				nowrap : false,
				...
				collapsible : false,
				singleSelect:true,
				url:'checkInStorage/selectStockItems.html',
				rownumbers:true,
				checkOnSelect:false,
			    columns:[[
				    {field:'name',title:'物资名称',width:90},    
				    {field:'size',title:'规格',width:130},     
				    {field:'prickle',title:'计量单位',width:90},     
				    {field:'count',title:'应归还数量',width:100},
				    {field:'scount',title:'未归还数量',width:100}
			    ]],
			    onBeforeLoad:function(param){
			    	if($("#sid").val()!="0"){
			    		param.id=$("#pid").val();
			...
			$('#inStorageInfo').datagrid({
			    ...
			    columns:[[
				    ...
				    {field:'name',title:'物资名称',width:90,editor:{
							type:'combobox',
							options:{
								url:'checkInStorage/selectMaterials.html',
								required:true,
								editable:false,
								valueField:'name',
								disabled:true,
								...
								onBeforeLoad: function(param){
									...
									var rows1 = $('#pinStorageInfo').datagrid("getRows");
									var str1="";
									for (var i = 0; i < rows1.length; i++) {
										if(rows1[i].scount != "0"){
											str1=rows1[i].id+";"+str1;
										}
									}
									param.type =type;
									param.str =str;
									param.parentstr=str1;
								}
							}
						}
				    },    
				    ...
				    {field:'count',title:'归还数量',width:90,editor:{
							type:'textbox',
							options:{
								required:true,
		...
		function add(){
			...
			if(editindex!=undefined){
				...
				$.messager.alert("操作提示", "请保存归还明细！", "error");
				return false;
			}
			...
			if(rows.length==0){
				$.messager.progress('close');
				$.messager.alert("操作提示", "请添加归还明细！", "error");
				return false;
			}else{
				for (var i = 0; i < rows.length; i++) {
			...
			var json = {
					...
					"parentid":$("#pcode").combobox("getValue"),
					"proposer":$("#proposer").combobox("getValue"),
					"proposertime":$("#proposerTime").datetimebox("getValue"),
					"items":data,
		...
		function edit(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存归还数据！", "error");
				return false;
			}
		...
		function apecct(){
			...
			if(isval){
				...
				$.messager.alert("操作提示", "请选择物资或正确填写归还数量！", "error");
				return false;
			}
			editindex=undefined;
		}
		function addOrder(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存归还明细！", "error");
				return;
			}
			var rows=$("#inStorageInfo").datagrid("getRows");
		...
		function reflush() {
			document.getElementById('checkInStorage.htmlifm').contentWindow.$('#checkInStorage_list')
					.datagrid('reload');
			document.getElementById('checkInStorage.htmlifm').contentWindow.$('#checkInStorage_list')
		...
		function commit(){
			$.messager.confirm('确认对话框', '您确定要提交吗？', function(r){
				if (r){
					$.messager.progress();
					$.post("checkInStorage/commit.html",{"id":$("#sid").val(),"parentId":$("#pid").val()},function(data){
						data = eval('(' + data + ')');
						$.messager.progress('close');
						if(data.result===true){
							reflush();
							...
							for (var i = 0; i < data.length; i++) {
								...
								$('#inStorageInfo').datagrid('updateRow',{
									index: index,
									row: {
										msg: '<span style="color:red;">未归还数量不足</span>'
									}
								});

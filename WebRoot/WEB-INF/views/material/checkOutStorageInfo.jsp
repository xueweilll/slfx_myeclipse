...
<body>
	...
	<form id="ff" method="post"
		...
		<div style="float: left;">
			<div style="float: left;"><label style="width: 70px;text-align: center;">外借单编号:</label><input class="easyui-textbox" id="code" data-options="width:230,disabled:true"  value="${stock.getCode()}" > </div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">外借审批人:</label><input id="handler"  type="text"  ></div>
		</div>
		<div style="float: left;">
			<div style="float:left;"><label style="width: 70px;text-align: center;">外借人:</label><input id="proposer"  type="text"  ></div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">外借时间:</label><input class="easyui-datetimebox" id="proposerTime" data-options="buttons:buttons,width:230,editable:false" required="true"  style="width:230" value="${datetime}" > </div>
		</div>
		...
		<div style="width:70px;text-align: center;margin-top:10px;margin-bottom: 10px;"><table id="inStorageInfo" cellspacing="0" cellpadding="0"></table></div>
		<div id="btn">
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#handler').combogrid({    
			    ...
			    width:230,
			    textField:'ename',    
			    url:'checkOutStorage/userBind.html',
			    required:true,
			    editable:false,
			    columns:[[    
			...
			if($("#storageInfo").val()=="0"||$("#storageInfo").val()=="3"||$("#storageInfo").val()=="2"){
			...
			$('#inStorageInfo').datagrid({
				...
				collapsible : false,
				singleSelect:true,
				url:'checkOutStorage/selectStockItems.html',
				rownumbers:true,
				queryParams: {
					id: $("#sid").val()
				},
			    ...
			    columns:[[
				    ...
				    {field:'name',title:'物资名称',width:90,editor:{
							type:'combobox',
							options:{
								url:'inStorage/selectMaterials.html',
								required:true,
								editable:false,
								disabled:true,
								valueField:'name',
				    ...
				    {field:'count',title:'外借数量',width:90,editor:{
							type:'textbox',
							options:{
								required:true,
		...
		function add(){
			...
			if(editindex!=undefined){
				...
				$.messager.alert("操作提示", "请保存外借明细！", "error");
				return false;
			}
			...
			if(rows.length==0){
				$.messager.progress('close');
				$.messager.alert("操作提示", "请添加外借明细！", "error");
				return false;
			}else{
				for (var i = 0; i < rows.length; i++) {
		...
		function edit(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存外借数据！", "error");
				return false;
			}
		...
		function apecct(){
			...
			if(isval){
				...
				$.messager.alert("操作提示", "请选择物资或正确填写外借数量！", "error");
				return false;
			}
			editindex=undefined;
		}
		function addOrder(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存外借明细！", "error");
				return;
			}
			var rows=$("#inStorageInfo").datagrid("getRows");
		...
		function reflush() {
			document.getElementById('checkOutStorage.htmlifm').contentWindow.$('#checkOutStorage_list')
					.datagrid('reload');
			document.getElementById('checkOutStorage.htmlifm').contentWindow.$('#checkOutStorage_list')
		...
		function commit(){
			$.messager.confirm('确认对话框', '您确定要提交吗？', function(r){
				if (r){
					$.messager.progress();
					$.post("checkOutStorage/commit.html",{"id":$("#sid").val()},function(data){
						data = eval('(' + data + ')');
						$.messager.progress('close');
						if(data.result===true){
							reflush();

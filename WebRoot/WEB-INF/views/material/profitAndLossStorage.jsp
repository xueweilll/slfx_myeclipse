...
<script type="text/javascript">
	...
	$(function() {
		$('#tt').tabs({
			...
			onSelect : function(title, index) {
				...
				$("#profitAndLossStorage_list").datagrid("unselectAll");
				$("#profitAndLossStorage_list").datagrid("load", {
					typeDate : index
				});
			}
		});
		$("#profitAndLossStorage_list").datagrid({
			width : 'auto',
			height : 'auto',
			pageSize : 20,
			nowrap : false,
			...
			collapsible : false,
			fit : true,
			url : 'profitAndLossStorage/bind.html',
			idField : 'id',
			singleSelect : true,
			queryParams: {
				typeDate : 0
			...
			columns:[[    
			          ...
			          {field:'handler',title:'处理人',width:140},    
			          {field:'handlertime',title:'处理时间',width:140},
		...
		var p = $("#profitAndLossStorage_list").datagrid('getPager');
		//console.info(p);
		$(p).pagination({
			pageList : [ 5, 10, 15, 20 ],
			beforePageText : '第',
	...
	function add() {
		showDialogWH("添加物资损益", "profitAndLossStorage/profitAndLossStorageInfo.html?id=0",650,510);
	}

	function edit() {
		var row = $("#profitAndLossStorage_list").datagrid("getSelected");
		if (row == null) {
		...
		showDialogWH("编辑物资损益", "profitAndLossStorage/profitAndLossStorageInfo.html?id=" + row.id,650,550);
	}
	...
	function distory() {
		...
		$.messager.confirm("删除提示", "您确定要执行删除吗？", function(data) {
			...
			if (data) {
				...
				$.ajax({
					type : 'POST',
					url : "profitAndLossStorage/delete.html",
					data : {
						id : row.id
					},
					success : function() {
						...
						$("#profitAndLossStorage_list").datagrid("unselectAll");
						$("#profitAndLossStorage_list").datagrid("reload");
					}
				});
			}
		});
	...
	function commit(){
		...
		showDialogWH("提交损益单", "profitAndLossStorage/profitAndLossStorageInfo.html?id=" + row.id+"&type=1",650,550);
	}
	function searchInfo(row){
		showDialogWH("查看损益明细", "profitAndLossStorage/profitAndLossStorageInfo.html?id=" + row.id+"&type=0",650,550);
	}
...
<body class="easyui-layout" id="cc">
	<table id="profitAndLossStorage_list" cellspacing="0" cellpadding="0"></table>
		<div id="tb" style="padding:5px;height:auto">
		<div id="tt" class="easyui-tabs" style="">
			<div title="未处理任务" data-options="closable:false"
		...
		<div class="cz_div">
			...
			损益编号:<input id="code" class="easyui-textbox" name="code"   

...
<body>
	...
	<input type="hidden" id="pid" value="${stock.getProposer()}">
	<input type="hidden" id="hid" value="${stock.getHandler()}">
	<form id="ff" method="post"
		style="margin: 10px; text-align: center;">
		<div style="float: left;">
			<div style="float: left;"><label style="width: 70px;text-align: center;">调拨单编号:</label><input class="easyui-textbox" id="code" data-options="width:230,disabled:true"   value="${stock.getCode()}" > </div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">调拨审批人:</label><input id="handler"  type="text" value="" ></div>
		</div>
		<div style="float: left;">
			<div style="float:left;"><label style="width: 70px;text-align: center;">申请人:</label><input id="proposer"  type="text" value="" ></div>
			<div style="float:right;"><label style="width: 70px;text-align: center;">申请时间:</label><input class="easyui-datetimebox" id="proposerTime" data-options="buttons:buttons,width:230,editable:false,width:230" required="true"  value="${datetime}" > </div>
		</div>
		<div style="float: left;margin-top:10px;">
			<label style="width: 70px;text-align: center;" >运往地点:</label><input class="easyui-textbox" id="address" data-options="multiline:true,validType:'length[0,66]',width:540,height: 60" value="${stock.getAddress()}" style="">
		</div>
		<div style="float: left;margin-top:10px;">
		...
		<div style="float: left;margin-top:10px;margin-bottom: 10px;"><table id="inStorageInfo" cellspacing="0" cellpadding="0"></table></div>
		<div id="btn" style="margin-top:10px;margin-bottom: 10px;">
	...
	<script type="text/javascript">
		...
		$(function(){
			$('#handler').combogrid({    
			    ...
			    width:230,
			    textField:'ename',    
			    url:'allotStorage/userBind.html',
			    required:true,
			    editable:false,
			    columns:[[    
			...
			$('#proposer').combogrid({    
			    ...
			    onLoadSuccess:function(node){
			    	if(node.rows.length!=0){
			    		for (var i = 0; i < node.rows.length; i++) {
				    		if(node.rows[i].id==$("#pid").val()){
				    			$("#proposer").combogrid("setValue", $("#pid").val());
				    		}
						}
			    	}
			    }
			...
			if($("#storageInfo").val()=="0"){
				...
				$("#proposer").combogrid({disabled:true,value:$("#pid").val(),onBeforeLoad:function(param){return false;}});
			...
			$('#inStorageInfo').datagrid({
				...
				collapsible : false,
				singleSelect:true,
				url:'allotStorage/selectStockItems.html',
				rownumbers:true,
				queryParams: {
					id: $("#sid").val()
				},
			    ...
			    columns:[[
				    ...
				    {field:'name',title:'物资名称',width:90,editor:{
							type:'combobox',
							options:{
								url:'allotStorage/selectMaterials.html',
								required:true,
								editable:false,
								disabled:true,
								valueField:'name',
				    ...
				    {field:'count',title:'调拨数量',width:90,editor:{
							type:'textbox',
							options:{
								required:true,
		...
		function add(){
			...
			if(editindex!=undefined){
				...
				$.messager.alert("操作提示", "请保存调拨明细！", "error");
				return false;
			}
			...
			if(rows.length==0){
				$.messager.progress('close');
				$.messager.alert("操作提示", "请添加调拨明细！", "error");
				return false;
			}else{
				for (var i = 0; i < rows.length; i++) {
			...
			var json = {
					...
					"address":$("#address").textbox("getValue").replace(/\s+/g,"")
			}
			$.ajax({
				type : "POST",
				url : "allotStorage/save.html",
				dataType : "text",
				data : {
		...
		function edit(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存调拨数据！", "error");
				return false;
			}
		...
		function apecct(){
			...
			if(isval){
				...
				$.messager.alert("操作提示", "请选择物资或正确填写调拨数量！", "error");
				return false;
			}
			editindex=undefined;
		}
		function addOrder(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存调拨明细！", "error");
				return;
			}
			var rows=$("#inStorageInfo").datagrid("getRows");
		...
		function reflush() {
			document.getElementById('allotStorage.htmlifm').contentWindow.$('#allotStorage_list')
					.datagrid('reload');
			document.getElementById('allotStorage.htmlifm').contentWindow.$('#allotStorage_list')
		...
		function commit(){
			$.messager.confirm('确认对话框', '您确定要提交吗？', function(r){
				if (r){
					$.messager.progress();
					$.post("allotStorage/commit.html",{"id":$("#sid").val()},function(data){
						data = eval('(' + data + ')');
						$.messager.progress('close');
						if(data.result===true){
							reflush();

...
<body>
	...
	<form id="ff" method="post"
		...
		<div style="float: left;">
			<div style="float: left;"><label style="width: 70px;text-align: center;">损益单编号:</label><input class="easyui-textbox" id="code" data-options="width:230,disabled:true"  value="${stock.getCode()}" > </div>
		</div>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('#inStorageInfo').datagrid({
				...
				collapsible : false,
				singleSelect:true,
				url:'profitAndLossStorage/selectStockItems.html',
				rownumbers:true,
				queryParams: {
					id: $("#sid").val()
				},
			    ...
			    columns:[[
				    ...
				    {field:'name',title:'物资名称',width:90,editor:{
							type:'combobox',
							options:{
								url:'profitAndLossStorage/selectMaterials.html',
								required:true,
								editable:false,
								disabled:true,
								valueField:'name',
					...
					{field:'count',title:'数量',width:90,editor:{
							type:'textbox',
							options:{
								required:true,
					...
					{field:'scount',title:'类型',width:60,formatter:function(value,row,index){
			        	  if(value==0){
			        		  return "溢出";
			        	  }else if(value==1){
			        		  return "损失";
							...
							type:'combobox',
							options:{
								url:'paramers/profitAndLossType.html',
								required:true,
								editable:false,
								valueField:'id',
								textField:'text'
					...
					{field:'msg',title:'提示信息',width:100}
			    ]],
		...
		function add(){
			...
			if(editindex!=undefined){
				...
				$.messager.alert("操作提示", "请保存损益明细！", "error");
				return false;
			}
			...
			if(rows.length==0){
				$.messager.progress('close');
				$.messager.alert("操作提示", "请添加损益明细！", "error");
				return false;
			}else{
				for (var i = 0; i < rows.length; i++) {
					data.push({"materialid":rows[i].id,"count":rows[i].count,"surpluscount":rows[i].scount});
				}
			}
			if (!isValid){
				$.messager.progress('close');
			...
			$.ajax({
				type : "POST",
				url : "profitAndLossStorage/save.html",
				dataType : "text",
				data : {
		...
		function edit(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存损益数据！", "error");
				return false;
			}
		...
		function addOrder(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存损益明细！", "error");
				return;
			}
			var rows=$("#inStorageInfo").datagrid("getRows");
		...
		function reflush() {
			document.getElementById('profitAndLossStorage.htmlifm').contentWindow.$('#profitAndLossStorage_list')
					.datagrid('reload');
			document.getElementById('profitAndLossStorage.htmlifm').contentWindow.$('#profitAndLossStorage_list')
		...
		function commit(){
			$.messager.confirm('确认对话框', '您确定要提交吗？', function(r){
				if (r){
					$.messager.progress();
					$.post("profitAndLossStorage/commit.html",{"id":$("#sid").val()},function(data){
						data = eval('(' + data + ')');
						$.messager.progress('close');
						if(data.result===true){
							reflush();

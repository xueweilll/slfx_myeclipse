	...
	<script type="text/javascript">
		$(function() {
			...
			$("#aduit").bind("click", function() {
				...
				$.ajax({
					type : "POST",
					url : "areareceiptaduit/aduit.html",
					dataType : "text",
					data : {
						"id":$("#Id").val(),
			...
			$("#downAccessory").bind("click",function(){
				var link="areareceiptaduit/downAccessory.html?filename="+myEncoder($("#Accessory").val());
		...
		function reflush() {
			//if($("#type").val() == "0"){
				document.getElementById('areareceiptaduit.htmlifm').contentWindow.$(
					'#receipt_list').datagrid('reload');
			/* }else if($("#type").val() == "1"){
				document.getElementById('receipt/bigAduit.htmlifm').contentWindow.$(
				'#receipt_list').datagrid('reload');
			} */
		}
	</script>

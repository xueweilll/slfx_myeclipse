...
<script type="text/javascript">
	...
	$(function() {
		$('#tt').tabs({
			...
			onSelect : function(title, index) {
				...
				$("#scrapStorage_list").datagrid("unselectAll");
				$("#scrapStorage_list").datagrid("load", {
					typeDate : index
				});
			}
		});
		$("#scrapStorage_list").datagrid({
			width : 'auto',
			height : 'auto',
			pageSize : 20,
			nowrap : false,
		...
		var p = $("#scrapStorage_list").datagrid('getPager');
		//console.info(p);
		$(p).pagination({
			pageList : [ 5, 10, 15, 20 ],
			beforePageText : '第',
	...
	function add() {
		showDialogWH("添加物资报废", "scrapStorage/scrapStorageInfo.html?id=0",650,510);
	}

	function edit() {
		var row = $("#scrapStorage_list").datagrid("getSelected");
		if (row == null) {
		...
		showDialogWH("编辑物资报废", "scrapStorage/scrapStorageInfo.html?id=" + row.id,650,550);
	}
			...
			if (data) {
				...
				$.ajax({
					...
					success : function() {
						...
						$("#scrapStorage_list").datagrid("unselectAll");
						$("#scrapStorage_list").datagrid("reload");
					}
				});
			}
		});
	...
	function commit(){
		...
		showDialogWH("提交报废单", "scrapStorage/scrapStorageInfo.html?id=" + row.id+"&type=1",650,550);
	}
	function searchInfo(row){
		showDialogWH("查看报废明细", "scrapStorage/scrapStorageInfo.html?id=" + row.id+"&type=0",650,550);
	}
...
<body class="easyui-layout" id="cc">
	<table id="scrapStorage_list" cellspacing="0" cellpadding="0"></table>
		...
		<div class="cz_div">
			...
			报废编号:<input id="code" class="easyui-textbox" name="code"   

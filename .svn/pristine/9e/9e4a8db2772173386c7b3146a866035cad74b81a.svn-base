...
public class PatrolPlanController extends BasicController {
	@Autowired
	...
	public PatrolPlanController() {
		super(PatrolPlanController.class);
	}
	...
	public ModelAndView index() {
		...
		mv.setViewName("patrol/patrolplan");
		return mv;
	}
	...
	//管理处制定计划
	  @RequestMapping(value = "/patrolplanlist", method = RequestMethod.POST)
		@ResponseBody
		public String patrolSpecialIssueList(int page, int rows, String starttime, String endtime,String type, String typeDate, HttpSession session) {
			String strJson="";
			page = (page == 0 ? 1 : page);
			rows = (rows == 0 ? 15 : rows);
			...
			List<PatrolSpecialIssue> patrolSpecialIssue=null;
			    patrolSpecialIssue=service.findByPage(map);
			    int total = service.pageCount(map);
			...
			gson = new GsonBuilder().registerTypeAdapter(PatrolSpecialIssue.class,
					new JsonSerializer<PatrolSpecialIssue>() {
						...
						public JsonElement serialize(PatrolSpecialIssue patrolSpecialIssue, Type typeOfSrc,
							...
							//String code = activitiUnitService.selectMaxCode("GC");
						//    patrolSpecialIssue.setCode(code);
							...
							if(patrolSpecialIssue.getCreatetime() != null){
	      ...
	      //  code = activitiUnitService.selectMaxCode("FS");
	  @RequestMapping(value = "patrolplaninfo", method = RequestMethod.GET)
		public ModelAndView patrolplanInfo(String id, HttpServletRequest req) throws Exception {
			ModelAndView mv = new ModelAndView();
			//修改页面
			if(!id.equals("0")){
				...
				mv.addObject("qf",patrolSpecialIssue.getPatrolSpecialFolw().getQf());
			}else{
				String code = activitiUnitService.selectMaxCode("FS");
			    mv.addObject("code",code);
			}
			mv.setViewName("patrol/patrolplaninfo");
			return mv;
		}
	
	  @RequestMapping(value = "patrolplaninfoheld", method = RequestMethod.GET)
		public ModelAndView patrolplanInfoheld(String id) throws Exception {
			...
			mv.setViewName("patrol/patrolplaninfoheld");
			return mv;
		}

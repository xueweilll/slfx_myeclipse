...
<script type="text/javascript">
	...
	$(function() {
		...
		var p = $("#patrolReport_list").datagrid('getPager');
		//console.info(p);
		$(p).pagination({
			pageList : [ 5, 10, 15, 20 ],
			beforePageText : '第',
	...
	function selectbig(){
		$("#patrolReport_list").datagrid('unselectAll');
		var isValid=$("#endtime").datetimebox("isValid");
		if(!isValid){
			return false;
		}
		$("#patrolReport_list").datagrid('load',{
	...
	function searchInfo(row){
		...
		showDialogWH("查看明细", "patrolReport/patrolReportInfo.html?jsonStr=" + str,650,500);
	}
...
<body class="easyui-layout" id="cc">
	<table id="patrolReport_list" cellspacing="0" cellpadding="0"></table>

...
<script type="text/javascript">
	$(function() {
		...
		$("#starttime").datetimebox({
			editable:false, //定义用户是否可以直接输入文本在框里
		...
		$("#patrolplanlist").datagrid({
			...
			collapsible : false,
			fit : true,
			url : 'patrolplan/patrolplanlist.html',
			queryParams : {
				type : "0",
				typeDate : "0"
			},
			...
			onDblClickRow:function(index,row){
				showDialogWH("查看签发信息",
						"patrolplan/patrolplaninfoheld.html?id=" + row.id, "430", "380");
		...
		$("#add").bind("click",function() {
			showDialogWH("添加签发信息", "patrolplan/patrolplaninfo.html?id=0", 450, 400);
		});
		//修改
		$("#edit").bind("click",function() {
			...
			/* }else if(row.stateValue!="0") {
				...
				showDialogWH("编辑签发信息","patrolplan/patrolplaninfo.html?id=" + row.id,450,400);
			}

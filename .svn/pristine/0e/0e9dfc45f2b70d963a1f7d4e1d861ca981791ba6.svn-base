...
<script type="text/javascript">
	$(function() {
		...
		$("#receipt_list").datagrid({
			...
			collapsible : false,
			fit : true,
			url : 'bigsurroundreceipt/pageBind.html',
			queryParams: {
				type:$("#type").val(),
				isAduit:"0",
				typeDate:"0"
			...
			onDblClickRow:function(index,row){				
				showDialogWH("编辑调度令接收单信息","areareceipt/receiptInfo.html?id=" + row.id +"&state=1&type=" + $("#type").val(), 420, 420);
			}
		});
		...
		$("#btnAdd").bind("click", function() {
			showDialogWH("添加调度令接收单信息", "bigsurroundreceipt/receiptInfo.html?id=0&state=0&type=" + $("#type").val(), 420, 420);
		});
		...
		$("#btnCommit").bind("click",function(){
			...
			$.ajax({
				type : 'POST',
				url : "bigsurroundreceipt/commit.html",
				data : {
					"id" : row.id
				},
		...
		$("#btnModify").bind("click",function() {
			...
			showDialogWH("编辑调度令接收单信息","bigsurroundreceipt/receiptInfo.html?id=" + row.id +"&state=0&type=" + $("#type").val(), 420, 420);
		});
		...
		$("#btnDelete").bind("click", function() {
			...
			$.messager.confirm("删除提示", "您确定要执行删除吗？", function(data) {
				if (data) {
					...
					$.ajax({
						type : 'POST',
						url : "bigsurroundreceipt/delete.html",
						data : {
							"id" : row.id
						},
						success : function(msg) {
	...
	function compulsoryDistory(){
		...
		$.messager.confirm("删除提示", "您确定要执行删除吗？", function(data) {
			if (data) {
				...
				$.ajax({
					type : 'POST',
					url : "bigsurroundreceipt/compulsorydelete.html",
					data : {
						"id" : row.id
					},
					success : function(msg) {

			...
			<table>
				...
				<tr>
				   <td>	<a id="add" class="easyui-linkbutton" data-options="iconCls:'icon-add'"
					onclick="addsuggestion()">添加审核意见</a></td>
	...
	<script type="text/javascript">
		$(function() {
			$("#tenancedetial_list").datagrid({
				url : 'tenanceShList/tenanceShInfoList.html?id='+$('#id').val(),
				rownumbers : true,
				width : 'auto',
				height : '200px',
				closed : false,
		...
		function agree(obj){
			...
			$.messager.confirm('确认对话框', '是否审核?', function(r){
				if (r){
					$.ajax({
						type:"POST",
					    url:"tenanceShList/agree.html",
					    dataType:"text",
					    data:{
					    	"id":$("#id").val(),
					    	"type":obj,
		...
		function reflush() {
			document.getElementById('tenanceShList.htmlifm').contentWindow.$(

...
<body>
	...
	<form id="ff" method="post"
		...
		<div style="float: left;">
			<div style="float: left;"><label style="width: 70px;text-align: center;">入库单编号:</label><input class="easyui-textbox" id="code" data-options="width:230,disabled:true"  value="${stock.getCode()}" > </div>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$('#inStorageInfo').datagrid({
				...
				collapsible : false,
				singleSelect:true,
				url:'inStorage/selectStockItems.html',
				rownumbers:true,
				queryParams: {
					id: $("#sid").val()
				},
			    ...
			    columns:[[
				    ...
				    {field:'count',title:'入库数量',width:90,editor:{
							type:'textbox',
							options:{
								required:true,
		...
		function add(){
			...
			if(editindex!=undefined){
				...
				$.messager.alert("操作提示", "请保存入库明细！", "error");
				return false;
			}
			...
			if(rows.length==0){
				$.messager.progress('close');
				$.messager.alert("操作提示", "请添加入库明细！", "error");
				return false;
			}else{
				for (var i = 0; i < rows.length; i++) {
		...
		function apecct(){
			...
			if(isval){
				...
				$.messager.alert("操作提示", "请选择物资或正确填写入库数量！", "error");
				return false;
			}
			editindex=undefined;
		}
		function addOrder(){
			if(editindex!=undefined){
				$.messager.alert("操作提示", "请先保存入库明细！", "error");
				return;
			}
			var rows=$("#inStorageInfo").datagrid("getRows");
		...
		function reflush() {
			document.getElementById('inStorage.htmlifm').contentWindow.$('#inStorage_list')
					.datagrid('reload');
			document.getElementById('inStorage.htmlifm').contentWindow.$('#inStorage_list')
		...
		function commit(){
			$.messager.confirm('确认对话框', '您确定要提交吗？', function(r){
				if (r){
					$.messager.progress();
					$.post("inStorage/commit.html",{"id":$("#sid").val()},function(data){
						data = eval('(' + data + ')');
						$.messager.progress('close');
						if(data.result){
							reflush();

...
/**
 * @ClassName: AllotStorageController
 * @Description: TODO(物资调拨操作)
 * @author 冯庆国
 * @date 2016年1月25日 下午4:06:26
 * 
 */
@Controller
@RequestMapping("allotStorage")
public class AllotStorageController extends BasicController {

	@Autowired
	private StockService service;

	public AllotStorageController() {
		super(AllotStorageController.class);
	...
	public ModelAndView index() {
		...
		mv.setViewName("/material/allotStorage");
		return mv;
	}

	/**
	 * @Title: checkInStorageInfo
	 * @Description: TODO(物资调拨明细编辑、新增、查看、提交)
	 * @param @param id
	 * @param @param type
	 * @param @return 设定文件
	 * @return ModelAndView 返回类型
	...
	@RequestMapping(value = "allotStorageInfo", method = RequestMethod.GET)
	public ModelAndView allotStorageInfo(String id, String type) {
		ModelAndView mv = new ModelAndView();
		...
		if (!id.equals("0")) {
			try {
				...
				mv.addObject("datetime",
						datetimeFormat.format(stock.getProposertime()));
			} catch (Exception e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		} else {
			stock.setCode(activitiUnitService.selectMaxCode("DB"));
		}
		...
		mv.setViewName("/material/allotStorageInfo");
		return mv;
	}

	/**
	 * @Title: bind
	 * @Description: TODO(物资调拨数据绑定)
	 * @param @param page
	 * @param @param rows
	 * @param @param code
	...
	@ResponseBody
	public String bind(int page, int rows, String code, String starttime,
		...
		gson = new GsonBuilder().registerTypeAdapter(Stock.class,
				new JsonSerializer<Stock>() {
					@Override
					public JsonElement serialize(Stock arg0, Type arg1,
						...
						if (arg0.getProposertime() != null) {
							json.addProperty("proposerTime", datetimeFormat
									.format(arg0.getProposertime()));
						}
						if (arg0.getHandletime() != null) {
							json.addProperty("handlertime",
									datetimeFormat.format(arg0.getHandletime()));
						} else {
						...
						if (arg0.getAddress() != null) {
							json.addProperty("address",
									arg0.getAddress().length() > 20 ? arg0
											.getAddress().substring(0, 10)
											+ "..." : arg0.getAddress());
						}
						if (arg0.getCasuse() != null) {
							json.addProperty("casuse",
									arg0.getCasuse().length() > 20 ? arg0
											.getCasuse().substring(0, 10)
											+ "..." : arg0.getCasuse());
						}
	...
	public String selectStockItems(String id) {
		...
		gson = new GsonBuilder().registerTypeAdapter(StockItems.class,
				new JsonSerializer<StockItems>() {
					@Override
					public JsonElement serialize(StockItems arg0, Type arg1,
						...
						json.addProperty("prickle", arg0.getMaterial()
								.getPrickleid());
	...
	public String commit(String id) {
		...
		try {
			...
			materialIds = storageService.outStorage(items, "3");
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (materialIds.size() == 0) {
			...
			String str = gson.toJson(materialIds);
			return "{'result':1,'msg':'" + str + "'}";
		}
	}

	/**
	 * @Title: delete
	 * @Description: TODO(物资调拨删除)
	 * @param @param id
	 * @param @return 设定文件
	 * @return String 返回类型
	 * @throws
		...
		gson = new GsonBuilder().registerTypeAdapter(User.class,
				new JsonSerializer<User>() {
					...
					public JsonElement serialize(User arg0, Type arg1,
						...
						json.addProperty("ephone", arg0.getEmployee()
								.getPhone());

...
<script type="text/javascript">
	...
	$(function() {
		$('#tt').tabs({
			...
			onSelect : function(title, index) {
				...
				$("#checkInStorage_list").datagrid("unselectAll");
				$("#checkInStorage_list").datagrid("load", {
					typeDate : index,
					type:0
				});
			}
		});
		$("#checkInStorage_list").datagrid({
			width : 'auto',
			height : 'auto',
			pageSize : 20,
			nowrap : false,
			...
			columns:[[    
			          ...
			          {field:'proposer',title:'归还人',width:140},    
			          {field:'proposerTime',title:'归还时间',width:140},  
		...
		var p = $("#checkInStorage_list").datagrid('getPager');
		//console.info(p);
		$(p).pagination({
			pageList : [ 5, 10, 15, 20 ],
			beforePageText : '第',
	...
	function add() {
		showDialogWH("添加物资归还", "checkInStorage/checkInStorageInfo.html?id=0",670,550);
	}

	function edit() {
		var row = $("#checkInStorage_list").datagrid("getSelected");
		if (row == null) {
		...
		showDialogWH("编辑物资归还", "checkInStorage/checkInStorageInfo.html?id=" + row.id,670,550);
	}
	...
	function distory() {
		...
		$.messager.confirm("删除提示", "您确定要执行删除吗？", function(data) {
			...
			if (data) {
				...
				$.ajax({
					type : 'POST',
					url : "checkInStorage/delete.html",
					data : {
						id : row.id
					},
					success : function() {
						...
						$("#checkInStorage_list").datagrid("unselectAll");
						$("#checkInStorage_list").datagrid("reload");
					}
				});
			}
		});
	...
	function commit(){
		...
		showDialogWH("提交归还单", "checkInStorage/checkInStorageInfo.html?id=" + row.id+"&type=1",670,550);
	}
	function searchInfo(row){
		showDialogWH("查看归还明细", "checkInStorage/checkInStorageInfo.html?id=" + row.id+"&type=0",670,550);
	}
...
<body class="easyui-layout" id="cc">
	<table id="checkInStorage_list" cellspacing="0" cellpadding="0"></table>
		...
		<div class="cz_div">
			...
			归还编号:<input id="code" class="easyui-textbox" name="code"   

  ...
  <head>
	...
	<script>
		$(function(){
			...
			$('#szpatrol').datagrid({
				url:'szpatrol/szpatrolList.html',
				/* method:'post', */
				queryParams : {
					all:0
				},
			...
			$('#tt').tabs({
				...
				onSelect : function(title, index) {
					...
					$('#szpatrol').datagrid('load',{
						all:index
						/* ,
						begintime:$('#begin').datetimebox('getValue'),
		...
		function Add(){
			showDialogWH("添加巡检上报", "szpatrol/szpatrolInfo.html?id=0",520,400);
		}
		function Edit(){
			...
			if(Item==null){
				...
				$.messager.alert("操作提示","该项已提交，不能编辑！","info");
			}else{
				showDialogWH("编辑巡检上报", "szpatrol/szpatrolInfo.html?id="+Item.id,520,400);
			}
		}
		function Delete(){
			var Item=$('#szpatrol').datagrid('getSelected');
		...
		function Report(){
			...
			if(Item==null){
				...
				showDialogWH("水政巡检上报", "szpatrol/szpatrolReport.html?id="+Item.id,520,400);
			}
		}
      	function Search(){
    ...
    <div id="cc" class="easyui-layout" style="width:100%;height:100%;">
        <div id="toolbar">
        	...
        	<div id="tool1" class="cz_div_title">
        		<a class="easyui-linkbutton" data-options="iconCls:'icon_commit',plain:false" onclick="Report()">提交</a>
        	</div>
        	<div id="tool2" class="cz_div_title">

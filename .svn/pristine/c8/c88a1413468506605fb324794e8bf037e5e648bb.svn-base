...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#psprojectreport_list").datagrid({
			...
			collapsible : false,
			fit : true,
			url : 'patrolSpecialmanagement1/bind.html',
			idField : 'id',
			singleSelect : true,
			queryParams: {
				typeDate : 0
	...
	function commit(){
		...
		showDialogWH("提交签发单", "patrolSpecialmanagement1/patrolSpecialmanagement1Info.html?isId=" + row.id+"&type=1",670,590);
	}
	function searchInfo(row){
		showDialogWH("查看签发单明细", "patrolSpecialmanagement1/patrolSpecialmanagement1Info.html?isId=" + row.id+"&type=0",670,590);
	}

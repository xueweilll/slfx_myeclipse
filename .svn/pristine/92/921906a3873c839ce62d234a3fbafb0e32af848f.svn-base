...
<script type="text/javascript">
	$(function() {
		...
		$("#employee_list").datagrid({
			...
			collapsible : false,
			fit : true,
			url : 'size/systemSize.html',
			remoteSort : false,
			idField : 'id',
			singleSelect : true,
			pagination : true,
			rownumbers : true,
			/* <th data-options="field:'name',width:120">规格名称</th>
			...
			columns:[[				    			        
				        {field:'name',title:'规格名称',width:fixWidth(0.2)},
...
/* 		$("#user").combobox({
		...
		$("#btnAdd").bind("click", function() {
			showDialogWH("添加规格信息", "size/sizeInfo.html?id=0", 440, 150);
		});
		$("#btnEdit").bind(
				"click",
				function() {
					...
					showDialogWH("修改规格信息", "size/sizeInfo.html?id=" + row.id,
							440, 150);
				});

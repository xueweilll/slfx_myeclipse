...
import com.benqzl.service.dispatch.ReceiptDispatchCallbackService;
...
public class CallBackController extends BasicController {
	...
	public CallBackController() {
		super(CallBackController.class);
	}
	...
	public ModelAndView callback() {
		...
		mv.setViewName("dispatch/rdcallback");
		return mv;
	}

	@RequestMapping(value = "callbackInfo", method = RequestMethod.GET)
	public ModelAndView callbackInfo(String rdid) {
		ModelAndView mv = new ModelAndView();
		...
		mv.setViewName("dispatch/rdcallbackInfo");
		return mv;
	}

	@RequestMapping(value = "callbackLookInfo", method = RequestMethod.GET)
	public ModelAndView callbackLookInfo(String rdid){
		ModelAndView mv = new ModelAndView();
		ReceiptDispatch rd = receiptDispatchService.selectByPk(rdid);
		mv.addObject("rd", rd);
		mv.addObject("launchtime",
		...
		mv.addObject("receiptetime",
				datetimeFormat.format(rd.getReceipt().getReceiptetime()));
		mv.setViewName("dispatch/rdcallbacklookInfo");
		return mv;
	}
	...
	public String save(String jsonStr, String type, HttpSession session) {
		...
		gson = new Gson();
		if ("0".equals(type)) {
			ReceiptDispatchCallback callback = gson.fromJson(jsonStr,
					ReceiptDispatchCallback.class);
			User user = (User) session.getAttribute("loginUser");
			UUID uuid = UUID.randomUUID();
			callback.setId(uuid.toString());
			if (user != null) {
				callback.setCreater(user.getUserid());
			}
			callback.setCreatetime(new Date());
			callback.setEdittime(new Date());
			HashMap<String, Object> map = new HashMap<String, Object>();
			map.put("id", callback.getRdid());
			map.put("trstate", 2);
			try {
				receiptDispatchService.insertCallback(callback);
				//if (rd.getState() == 4) {
	...
	public String pageBind(HttpServletRequest request, int page, int rows,
		...
		logger.info("this pageBind list strJson is " + strJson);

		return strJson;
	}

	@Autowired
	ReceiptDispatchCallbackService receiptDispatchCallbackService; 

	@RequestMapping(value = "cbListBind", method = RequestMethod.POST)
	@ResponseBody
	public String cbListBind(String rdid){
		List<ReceiptDispatchCallback> list = receiptDispatchCallbackService.selectByRDID(rdid);
		gson = new GsonBuilder().registerTypeAdapter(ReceiptDispatchCallback.class,
				new JsonSerializer<ReceiptDispatchCallback>() {
					@Override
					public JsonElement serialize(ReceiptDispatchCallback src,
							Type typeOfSrc, JsonSerializationContext context) {
						JsonObject o = new JsonObject();
						o.addProperty("id", src.getId());
						o.addProperty("rdid", src.getRdid());
						o.addProperty("callbackpeople", src.getCallbackpeople());
						o.addProperty("callbackunit", src.getCallbackunit());
						o.addProperty("callbacktime", datetimeFormat.format(src.getCallbacktime()));
						o.addProperty("operater", src.getOperateUser().getUsername());
						o.addProperty("callback", src.getCallback());
